...
/**
     ...
     *
     * @author A.Liapis (Original author), A. Hartzen (2015 modifications), O. Losada (2016 modifications)
     */
    public class QTable {
        /**
         ...
         */
        HashMap<String, int[]> exploreTable;
        /**
        ...
        QTable(int actionRange){
            ...
            exploreTable = new HashMap<String, int[]>();
        }
        ...
        int getNextAction(char[] map){
            ...
            if(randomGenerator.nextFloat()<explorationChance){
                ...
                prevAction=explore(map);
            } else {
            	System.out.println("Exploit");
                prevAction=getBestAction(map);
            }
            // Update visit count
            updateVisitCount(getMapString(prevState), prevAction);
            return prevAction;
        }
        
        void updateVisitCount(String state, int action) {
        	int[] visits = getVisitCount(state);
        	visits[action] += 1;
        	exploreTable.put(state, visits);
        }
        
        int[] getVisitCount(String state) {
        ...
        void updateQvalue(int reward, char[] map){
        	...
        	float futureQValues[] = getActionsQValues(map);
        	...
        	table.put(getMapString(prevState), qValues);
        }
        ...
        void printQtable(){
            ...
            while (iterator.hasNext()) {
               char[] key = iterator.next().toCharArray();
